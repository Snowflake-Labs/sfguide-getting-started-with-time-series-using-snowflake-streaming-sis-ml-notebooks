...
st.session_state["metric_wrap"] = wrap
...
stat_queries = {
    ...
    'Min': "SELECT TAGNAME, TO_TIMESTAMP('{end_ts}') AS TIMESTAMP, MIN(VALUE) AS VALUE FROM TS_TAG_READINGS WHERE TIMESTAMP >= '{start_ts}' AND TIMESTAMP < '{end_ts}' AND TAGNAME IN {tag_tuple} GROUP BY TAGNAME",
    'Max': "SELECT TAGNAME, TO_TIMESTAMP('{end_ts}') AS TIMESTAMP, MAX(VALUE) AS VALUE FROM TS_TAG_READINGS WHERE TIMESTAMP >= '{start_ts}' AND TIMESTAMP < '{end_ts}' AND TAGNAME IN {tag_tuple} GROUP BY TAGNAME",
