/*
Private key is setup in Github Codespace Devcontainer
*/
...
-- Ingestion WH
CREATE WAREHOUSE IF NOT EXISTS HOL_INGEST_WH WITH WAREHOUSE_SIZE = XSMALL AUTO_SUSPEND = 60 AUTO_RESUME = TRUE INITIALLY_SUSPENDED = TRUE COMMENT = 'Ingestion Warehouse' ENABLE_QUERY_ACCELERATION = TRUE;
-- Transform WH
...
-- Reporting WH
CREATE WAREHOUSE IF NOT EXISTS HOL_REPORT_WH WITH WAREHOUSE_SIZE = XSMALL AUTO_SUSPEND = 60 AUTO_RESUME = TRUE INITIALLY_SUSPENDED = TRUE COMMENT = 'Reporting Warehouse' ENABLE_QUERY_ACCELERATION = TRUE;
-- HOL Database
...
CREATE SCHEMA IF NOT EXISTS HOL_TIMESERIES.ANALYTICS WITH MANAGED ACCESS COMMENT = 'HOL Time Series Reporting schema.';

-- Grant HOL role object access
GRANT USAGE ON DATABASE HOL_TIMESERIES TO ROLE ROLE_HOL_TIMESERIES;
GRANT ALL ON WAREHOUSE HOL_INGEST_WH TO ROLE ROLE_HOL_TIMESERIES;
